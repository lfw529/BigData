package com.lfw.practice;

import com.lfw.pojo.ItemCount;
import com.lfw.pojo.UserBehavior;
...
public class ItemCountTopN {
    public static void main(String[] args) throws Exception {
        ...
        //2.读取文本数据
        DataStreamSource<String> readTextFile = env.readTextFile("flink/src/main/resources/UserBehavior.csv");
        ...
        // <p>到达的数据使用给定的聚合函数增量聚合。这意味着窗口函数在调用时通常只有一个要处理的值。 @param aggFunction 用于增量聚合的聚合函数。
    ...
    public static class ItemCountWindowFunc implements WindowFunction<Integer, ItemCount, Long, TimeWindow> {
        ...
        public void apply(Long itemId, TimeWindow window, Iterable<Integer> input, Collector<ItemCount> out) throws Exception {
            ...
            //输出数据
            out.collect(new ItemCount(itemId, new Timestamp(window.getEnd()).toString(), count));
        }
    }
