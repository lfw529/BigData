...
public class EventTimeTumblingCustomerPeriod {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<WaterSensor> waterSensor2DS = env.socketTextStream("hadoop102", 7777)
                .map(data -> {
                    String[] split = data.split(",");
                    return new WaterSensor(split[0], Long.parseLong(split[1]), Integer.parseInt(split[2]));
                });
        //3.提取数据中的时间戳字段
        WatermarkStrategy<WaterSensor> waterSensor2WatermarkStrategy = new WatermarkStrategy<WaterSensor>() {
            @Override
        ...
        SingleOutputStreamOperator<WaterSensor> waterSensor2SingleOutputStreamOperator = waterSensor2DS.assignTimestampsAndWatermarks(waterSensor2WatermarkStrategy);
        //4.按照id分组
        KeyedStream<WaterSensor, String> keyedStream = waterSensor2SingleOutputStreamOperator.keyBy(WaterSensor::getId);
        //5.开窗
