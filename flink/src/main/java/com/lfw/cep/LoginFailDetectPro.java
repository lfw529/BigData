...
public class LoginFailDetectPro {
    public static void main(String[] args) throws Exception {
        ...
        KeyedStream<LoginCEP, String> stream = env.fromElements(
                        ...
                        WatermarkStrategy.<LoginCEP>forBoundedOutOfOrderness(Duration.ofSeconds(3))
                                .withTimestampAssigner(new SerializableTimestampAssigner<LoginCEP>() {
                                            @Override
        ...
        Pattern<LoginCEP, LoginCEP> pattern = Pattern.<LoginCEP>begin("fail")    // 第一个登录失败事件
                .where(new SimpleCondition<LoginCEP>() {
                    @Override
                    public boolean filter(LoginCEP loginEvent) throws Exception {
        ...
        SingleOutputStreamOperator<String> warningStream = patternStream
                .process(new PatternProcessFunction<LoginCEP, String>() {
                    @Override
                    public void processMatch(Map<String, List<LoginCEP>> match, Context ctx, Collector<String> out) throws Exception {
                        // 提取三次登录失败事件
                        LoginCEP firstFailEvent = match.get("fail").get(0);
                        LoginCEP secondFailEvent = match.get("fail").get(1);
                        LoginCEP thirdFailEvent = match.get("fail").get(2);
