package com.lfw.tolerance.demo;
...
/**
 * flink 的端到端精确一次容错能力测试
 * <p>
 * 从 kafka 读数据（里面有 operator-state 状态）
 * <p>
 * 处理过程中用到了带状态的 map 算子（里面用了 keyed-State 状态，逻辑：输入一个字符串，变大写拼接此前字符串，输出）
 * <p>
 * 用 exactly-once 的 mysql-sink 算子输出数据（并附带主键的幂等特性）
 * <p>
 * 测试用的 kafka-topic：
 * [lfw@hadoop102 ~]# kafka-topics.sh --create --topic eos --partitions 1 --replication-factor 1 --bootstrap-server hadoop102:9092
 * <p>
 * 测试用的输入数据：
 * a
 * b
...
public class ToleranceSideToSideTest {
    public static void main(String[] args) throws Exception {
        ...
        String pathCk = new File("").getCanonicalPath() + "/flink/output/eos_ckpt";
        ...
        KafkaSource<String> sourceOperator = KafkaSource.<String>builder()
                ...
                // kafkaSource 的做状态 checkpoint 时，默认会向 __consumer_offsets 提交一下状态中记录的偏移量
                // 但是，flink 的容错并不优选依赖 __consumer_offsets 中的记录，所以可以关闭该默认机制
        ...
        SinkFunction<String> exactlyOnceJdbcSink = JdbcSink.exactlyOnceSink(
                ...
                new SerializableSupplier<XADataSource>() {
                    ...
                    public XADataSource get() {
                        ...
                        xaDataSource.setUrl("jdbc:mysql://hadoop102:3306/flink_sink");
