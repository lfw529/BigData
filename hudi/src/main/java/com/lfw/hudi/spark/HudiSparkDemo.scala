...
object HudiSparkDemo {
  ...
  def queryData(spark: SparkSession, path: String): Unit = {
    ...
    //    		tripsDF.printSchema()   //打印结构
    //    		tripsDF.show(10, truncate = false)  //显示10条
  ...
  def queryDataByTime(spark: SparkSession, path: String): Unit = {
    ...
    val df1 = spark.read
      .format("hudi")
      .option("as.of.instant", "20220726232510623")
      .load(path)
    ...
    val df2 = spark.read
      .format("hudi")
      .option("as.of.instant", "2021-12-25 15:20:16.200")
      .load(path)
      .sort(col("_hoodie_commit_time").desc)
    df2.printSchema()
  ...
  def deleteData(spark: SparkSession, table: String, path: String): Unit = {
    ...
    val dataframe = tripsDF.limit(15).select($"uuid", $"partitionpath")  //可以根据条件筛选
    import org.apache.hudi.QuickstartUtils._
