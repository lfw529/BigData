...
/**
 * 从 kafka 的 topic-a 中读数据，处理 (把读到的数据转大写)，处理结果写回 kafka 的 topic-b
 * 利用 kafka 自身的事务机制，来实现 端到端的 eos 语义
 * 核心点：让消费端的偏移量记录更新 和 生产端的数据落地，绑定在一个事务中
 *
 * 测试流程：
 * 1.创建 topic-a, topic-b
 * 2.代码中埋入一个设计好的异常
 * 3.运行程序，往 topic-a 写入数据，从 topic-b 观察结果，看发生异常之后，topic-b 中是否能读到未提交事务的结果数据。
 */
public class TransactionEOS {
