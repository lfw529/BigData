...
# 具体书写案例：
# /opt/dw/data-etl-warehouse/tools/sqoop_import.sh -f /opt/dw/data-etl-warehouse/ods/tbl_conf/ods_order_info.conf -t 2021-04-22 -m 10
...
while getopts "f:t:b:m:x:d:h" opt; do
  case $opt in
  # -f 之后的参数： /opt/dw/data-etl-warehouse/ods/tbl_conf/ods_order_info.conf
  f)
    tbl_conf=$OPTARG
    ;;
  # -t 之后的参数：2021-04-22
...
# 从文件名中截取表名，例如：ods_order_info.conf  --> ods_order_info
...
#run sqoop import with param: /opt/dw/data-etl-warehouse/ods/tbl_conf/ods_order_info.conf 2023-06-03 1 10
...
do
    ...
    # jq 安装命令：
    db_info=`cat ${db_mysql} | jq ". | map(select(.InstanceName == \"${db_inst_i}\"))[0]"`         #查询到 map 是遍历器，select 是命中目标，获取第一个 db_info 的信息
    ...
    connect_str="jdbc:mysql://${mysql_ip}:${mysql_port}/${mysql_db}?tinyInt1isBit=false&useSSL=false"  # 拼接 sql-url
