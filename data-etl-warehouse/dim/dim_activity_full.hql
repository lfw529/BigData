...
CREATE EXTERNAL TABLE if not exists dim.dim_activity_full
(
...
-- Comment: dim层-活动维度-交易域
-- ********************************************************************
insert overwrite table dim.dim_activity_full partition(dt='${hiveconf:etl_date}')
select
    rule.id,
    info.id,
    activity_name,
