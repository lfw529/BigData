...
-- Comment: dim层-优惠券维度-交易域
-- ********************************************************************
insert overwrite table dim.dim_coupon_full partition(dt='${hiveconf:etl_date}')
select
    id,
    coupon_name,
    coupon_type,
...
(
    select
        ...
        dic_name
    from ods.ods_base_dic_full
    where dt='${hiveconf:etl_date}' and parent_code='33'
)range_dic
