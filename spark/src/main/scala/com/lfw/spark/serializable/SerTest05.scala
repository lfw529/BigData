...
object SerTest05 {
  def main(args: Array[String]): Unit = {
    ...
    //不再 Driver 端初始化 RuleObjectSer 或 RuleClassSer
    //函数是在 Driver 定义的
    val func = (line: String) => {
      val fields = line.split(",")
      val id = fields(0).toInt
      val code = fields(1)
      val name = RuleObjectNotSer.rulesMap.getOrElse(code, "未知") //闭包
      //获取当前线程ID
      ...
      (id, code, name, treadId, partitionId, host, RuleObjectNotSer.toString)
    }
